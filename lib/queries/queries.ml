    ...
    if p_nodes = [] then false
      ...
      let f_callers =
        Option.value ~default:[]
          (Option.map (Graph.get_callers graph) (f_name))
      in
      ...
      let f_returners = Option.bind f_name (Hashtbl.find_opt graph.returners) |> Option.value ~default:[] in
      (* Format.printf "f_returners: %a@." pp_nodes f_returners; *)
      List.exists (fun arg -> is_reachable graph arg exported_locs) (f_args @ f_returners)
