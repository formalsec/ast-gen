...
let rec is_reachable (graph : Graph.t) (node : Node.t)
  ...
  if
    p_nodes
    = [] (* check if any of the arguments of the function call is tainted *)
  then false
  else if
    List.mem f_node_loc exported_locs
    (* check if the function where the call is called is exported *)
  then true
  else
    let f_callers =
      Option.value
