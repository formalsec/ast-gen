...
module Id = struct
  ...
  let equal (func_id : t) (func_id' : t) =
    Int.equal func_id.uid func_id'.uid
  ...
  let to_string (func_id : t) : string =
    "(" ^ string_of_int func_id.uid ^ ", " ^ func_id.name ^ ")"
end
...
module rec Info : sig
  ...
  let create (size : int) : t =
    { top_lvl = ref []; functions = FuncTable.create size }

  let find (info : t) : Id.t -> info = FuncTable.find info.functions

  let find_opt (info : t) : Id.t -> info option =
    FuncTable.find_opt info.functions

  let replace (info : t) : Id.t -> info -> unit =
    FuncTable.replace info.functions
  ...
  let remove (info : t) (func_id : Id.t) : unit =
    ...
    FuncTable.filter_map_inplace
      (fun _ info ->
        Some
          { info with
            nested = List.filter Fun.(not << Id.equal func_id) info.nested
          } )
      info.functions

  let iter (f : Id.t -> info -> unit) (info : t) : unit =
    FuncTable.iter f info.functions
  ...
  let get_func_id (info : t) (func_name : string) (parent_id : Id.t option) :
    Id.t option =
    let context =
  ...
  let add (info : t) (func_id : Id.t) (parent_id : Id.t option)
    (params' : string list) : unit =
    (* remove previous version if it exists *)
    let prev_definition = get_func_id info (Id.get_name func_id) parent_id in
    Option.apply ~default:() (remove info) prev_definition;

    (* add found node information *)
    let func_info : info = { params = params'; nested = [] } in
...
module Context = struct
  ...
  let visit (context : t) (id : Id.t) : t =
    { context with path = id :: context.path }

  let get_current_function (context : t) () : Id.t option =
    List.nth_opt context.path 0
  ...
  let get_func_id (context : t) (func_name : string) : Id.t option =
    ...
    let rec aux (path : Id.t list) : Id.t option =
      ...
      else if Option.is_some parent_id then aux (List.tl path)
        (* function not found yet but there is more parents to search *)
      ...
      ~default:
        ( lazy
          (failwith
             ...
             ^ " is not definied in the given context" ) ) )
