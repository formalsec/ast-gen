...
module Js = struct
  ...
  and print_stmt (stmt : m Statement.t) (identation : int) : string =
    ...
    | (_, If { test; consequent; alternate }) ->
      ...
      let alternate' =
        Option.apply ~default:"\n"
          (fun alternate ->
            " else {\n"
            ^ print_stmts alternate new_identation
            ^ identation_str
            ^ "}\n" )
          alternate in
    ...
    | (_, Try { body; finalizer; handler }) ->
      ...
      let handler' =
        Option.apply ~default:""
          (print_handler identation identation_str)
          handler in
      let finalizer' =
        Option.apply ~default:"\n"
          (fun fin ->
            identation_str
            ^ "finally {\n"
            ^ print_stmts fin new_identation
            ^ identation_str
            ^ "}\n" )
          finalizer in
    ...
    | (_, VarDecl { kind; id }) ->
      let kind' =
        match kind with Var -> "var " | Let -> "let " | Const -> "const " in
      let id' = print_identifier id in
      identation_str ^ kind' ^ id' ^ ";\n"
    | (_, Return { argument; _ }) ->
      let argument' = Option.apply ~default:"" print_expr argument in
    ...
    | (_, Break { label }) ->
      let label' =
        Option.apply ~default:"" Fun.(( ^ ) " " << print_identifier) label in
    ...
    | (_, ExportNamedDecl { local; exported; all; source }) ->
      let exported' =
        Option.apply ~default:"" Fun.(( ^ ) " as " << print_identifier) exported
      in
      let source' =
        Option.apply ~default:""
          (fun source -> " from \"" ^ source ^ "\"")
          source in
    ...
    | (_, ImportDecl (Specifier { source; local; remote; namespace })) ->
      let local' =
        Option.apply ~default:"" Fun.(( ^ ) " as " << print_identifier) local
      in
      let remote' =
        if namespace then "*" else print_identifier (Option.get remote) in
    ...
    | (_, AssignBinary { left; operator; opLeft; opRight; _ }) ->
      ...
      let opRight' = print_expr opRight in

      identation_str ^ left' ^ " = " ^ opLeft' ^ operator' ^ opRight' ^ ";\n"
    ...
    | (_, Yield { left; argument; _ }) ->
      let left' = print_identifier left in
      let argument' =
        Option.apply ~default:"" Fun.(( ^ ) " " << print_expr) argument in
    ...
    | (_, StaticUpdate { obj; property; right; is_literal; _ }) ->
      ...
      if is_literal then
        ...
        ^ right'
        ^ ";\n"
      else identation_str ^ obj' ^ "." ^ property ^ " = " ^ right' ^ ";\n"
    | (_, DynmicUpdate { obj; property; right; _ }) ->
      let obj' = print_expr obj in
      let property' = print_expr property in
      let right' = print_expr right in
      identation_str ^ obj' ^ "[" ^ property' ^ "] = " ^ right' ^ ";\n"
    | (_, StaticDelete { left; obj; property; is_literal; _ }) ->
      ...
      if is_literal then
        ...
        ^ property
        ^ "\"];\n"
      else identation_str ^ left' ^ " = delete " ^ obj' ^ "." ^ property ^ ";\n"
    | (_, DynamicDelete { left; obj; property; _ }) ->
      let left' = print_identifier left in
      let obj' = print_expr obj in
      let property' = print_expr property in
      identation_str ^ left' ^ " = delete " ^ obj' ^ "[" ^ property' ^ "];\n"
    | (_, StaticLookup { left; obj; property; is_literal; _ }) ->
      let left' = print_identifier left in
      let obj' = print_expr obj in
      if is_literal then
        identation_str ^ left' ^ " = " ^ obj' ^ "[\"" ^ property ^ "\"]" ^ ";\n"
      else identation_str ^ left' ^ " = " ^ obj' ^ "." ^ property ^ ";\n"
    | (_, DynmicLookup { left; obj; property; _ }) ->
      ...
      identation_str ^ left' ^ " = " ^ obj' ^ "[" ^ property' ^ "];\n"
    | (_, UseStrict _) -> identation_str ^ "\"use strict\";\n"
  ...
  and print_expr (expr : m Expression.t) : string =
    ...
    | (_, TemplateLiteral { quasis; exprs }) ->
      let quasis' =
        List.map
          (fun (_, { Expression.TemplateLiteral.Element.value = { raw; _ }; _ }) ->
            raw )
          quasis in
      let exprs' = List.map print_expr exprs in

      let quasi_expr =
        List.map
          (fun (raw, expr) -> raw ^ if expr != "" then "${" ^ expr ^ "}" else "")
          (List.combine quasis' (exprs' @ [ "" ])) in
  ...
  and print_stmts (stmts : m Statement.t list) (identation : int) : string =
    String.concat "" (List.map (Fun.flip print_stmt identation) stmts)

  and print_identifier (identifier : m Identifier.t) =
    Fun.(print_expr << Identifier.to_expr) identifier

  and print_case (_, { Statement.Switch.Case.test; consequent })
      (identation : int) : string =
    let identation_str = String.make identation ' ' in
    let test' =
      Option.apply ~default:"default: \n"
        (fun test -> "case " ^ print_expr test ^ ": \n")
        test in
    let new_identation = identation + spaces_per_identation in
    let consequent' = print_stmts consequent new_identation in
  ...
  and print_param (_, { Statement.AssignFunction.Param.argument; default }) :
      string =
    let argument' = print_identifier argument in
    let default' =
      Option.apply ~default:"" (fun def -> " = " ^ print_expr def) default in
    argument' ^ default'
  ...
  and print_handler (identation : int) (identation_str : string)
      ((_, { param; body }) : 'M Statement.Try.Catch.t) : string =
    let param' =
      Option.apply ~default:""
        (fun param -> "(" ^ print_identifier param ^ ")")
        param in
    let new_identation = identation + spaces_per_identation in
    let body' = print_stmts body new_identation in
