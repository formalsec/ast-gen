  ...
  let analyse (config : t) (_ : State.t) (statement : m Statement.t) : t = 
    match statement with
      | _, AssignSimple {left; right} -> 
        ...
        map_default (fun right ->
          ...
          map_default (fun (sink : functionSink) ->
            ...
            Config.add_function_sink config { vuln_t = sink.vuln_t; sink = alias; args = sink.args}
          ) config sink;
      ...
      | _, StaticLookup {left; _object; property; _} -> 
        ...
        map_default (fun obj ->
          let (vuln_t, package) = Config.get_package_sink_info config obj property in 
          map_default (fun (package : package) ->
            let alias = Identifier.get_name left in 
            Config.add_function_sink config { vuln_t; sink = alias; args = package.args}
          ) config package
        ) config _object;
