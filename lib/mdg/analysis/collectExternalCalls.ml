...
module Analysis : AbstractAnalysis.T = struct
  ...
  let create_ext_ref (module' : string) (properties : property list) :
      external_reference =
    { _module = module'; properties }

  let add_property (ext_ref : external_reference) (property : property) :
      external_reference =
    { ext_ref with properties = ext_ref.properties @ [ property ] }

  (* handle external calls collection *)
  let add_ext_call (info : t) (call : LocationSet.t)
      (ext_ref : external_reference) : t =
    ExternalReferences.T.replace info.calls call ext_ref;
    info

  (* handle external references collection *)
  let add_ext_ref (info : t) (loc : LocationSet.t) (ext_ref : external_reference)
      : t =
  ...
  let analyse (info : t) (state : State.t) (statement : m Statement.t) : t =
    ...
    | (_, AssignFunCall { left; callee; arguments; id_call; _ }) ->
      if Identifier.name callee = "require" then
        (* get module name *)
        ...
        | Some (_, Literal { value = String module_name; _ }) ->
          let module_name =
            if String.starts_with ~prefix:"./" module_name then
              String.sub module_name 2 (String.length module_name - 2)
            else module_name in

          let loc = eval_expr (Identifier.to_expr left) in
        ...
        let callee = eval_expr (Identifier.to_expr callee) in
        let external_reference = get_ext_ref info callee in
        Option.apply ~default:info
          (fun external_reference -> add_ext_call info l_call external_reference)
          external_reference
    | (_, AssignMetCallStatic { obj; property; id_call; _ }) ->
      (* check if object callee is an external reference *)
      let l_call = LocationSet.singleton (alloc id_call) in
      let callee = eval_expr obj in
      let external_reference = get_ext_ref info callee in
      Option.apply ~default:info
        (fun external_reference ->
  ...
  let finish (info : t) : AnalysisType.t =
    AnalysisType.CollectExternalCalls info.calls
end
