...
module GraphConstrunction (Auxiliary : AbstractAnalysis.T) = struct
  ...
  (* ------- I N I T I A L I Z E ------- *)
  let initialize_functions (state : State.t) (funcs_info : Function.Info.t) :
      unit =
    let init_func_header (state : State.t) (func : Function.Id.t)
        (info : Function.Info.func) : unit =
  ...
  and analyse_simple (state : State.t) (analysis : Auxiliary.t ref)
    ...
    let get_curr_func () = Function.Context.curr_func contx in
    ...
    | (_, AssignFunction { left; id; body; _ }) ->
      let func_id : Function.Id.t = { uid = id; name = Identifier.name left } in
    ...
    | (loc, StaticLookup { left; obj; property; id; _ }) ->
      let add_node' : location -> unit =
       fun abs_loc ->
        add_node abs_loc (property_lookup_name left obj property) loc in

      let _L = eval_expr obj in
    ...
    | (loc, DynmicLookup { left; obj; property; id }) ->
      let add_node' : location -> unit =
       fun abs_loc -> add_node abs_loc (property_lookup_name left obj "*") loc
      in

      let (_L1, _L2) = (eval_expr obj, eval_expr property) in
    ...
    | (loc, StaticUpdate { obj; property; right; id; _ }) ->
      let add_node' : location -> unit =
       fun abs_loc ->
        add_node abs_loc (Option.value (Expression.id_opt obj) ~default:"*") loc
      in

      let (_L1, _L2) = (eval_expr obj, eval_expr right) in
      let _L1' = new_version store _L1 property id add_node' in
      LocationSet.apply
        (fun l_1 ->
          LocationSet.apply
            (fun l_2 -> Graph.add_prop_edge graph l_1 l_2 (Some property))
            _L2 )
        _L1'
    (* -------- D Y N A M I C   P R O P E§ R T Y   U P D A T E -------- *)
    | (loc, DynmicUpdate { obj; property; right; id }) ->
      let add_node' : location -> unit =
       fun abs_loc ->
        add_node abs_loc (Option.value (Expression.id_opt obj) ~default:"*") loc
      in
      ...
      let (_L1, _L2, _L3) =
        (eval_expr obj, eval_expr property, eval_expr right) in

      let _L1' = new_version' store _L1 _L2 id add_node' in
      LocationSet.apply
        (fun l_1 ->
          LocationSet.apply
            (fun l_3 -> Graph.add_prop_edge graph l_1 l_3 None)
            _L3 )
        _L1'
    (* -------- C A L L -------- *)
    ...
    | (loc, AssignFunCall { left; callee; arguments; id_call; id_retn; _ }) ->
      ...
      let f = Identifier.name callee in
      let f_id = get_func_id f in

      (* node information *)
      let call_node = add_cnode l_call f loc in
      add_node l_retn (Identifier.name left) loc;
    ...
    | ( loc
      , AssignMetCallStatic
          { left; obj; property; arguments; id_call; id_retn; _ } ) ->
      analyse_method_call state loc left obj property arguments id_call id_retn
    | (loc, AssignMetCallDynmic { left; obj; arguments; id_call; id_retn; _ })
      ->
      analyse_method_call state loc left obj "*" arguments id_call id_retn
    (* -------- R E T U R N -------- *)
  ...
  and property_lookup_name (left : m Identifier.t) (obj : m Expression.t)
      (property : string) : string =
    let obj_name = Option.value (Expression.id_opt obj) ~default:"*" in
    let obj_prop = obj_name ^ "." ^ property in
    if Identifier.is_generated left then obj_prop
    else Identifier.name left ^ ", " ^ obj_prop

  and analyse_method_call (state : State.t) (loc : Location.t)
      (left : m Identifier.t) (obj : m Expression.t) (property : property)
    ...
    let add_call_edge = Graph.add_call_edge graph in
    let add_ref_call_edge = Graph.add_ref_call_edge graph in
    let get_curr_func () = Function.Context.curr_func state.context in
    let lookup = Graph.lookup graph in

    let _Lss = List.map eval_expr arguments in

    let _Lthis = eval_expr obj in
    let l_call = alloc id_call in
    let l_retn = alloc id_retn in
        ...
        Option.apply ~default:()
          (fun ref ->
            ...
            if List.length ref.properties = 1 then
              ...
              let sink_info =
                Config.get_package_sink config package_name method_name in
              Option.apply ~default:()
...
and add_taink_sink (graph : Graph.t) (loc : location) (node : Graph.Node.t)
...
let rec buildExportsObject (state : State.t) (info : buildExportsObject) :
  ...
  Option.apply ~default:()
    (fun loc -> exportsObject := constructobj state loc)
    info.moduleExportsObject;

  (* assignments to the properties of module.exports *)
  Hashtbl.iter
    (fun property loc ->
      let object' = constructobj state loc in
      exportsObject :=
...
and constructobj (state : State.t) (loc : LocationSet.t) : ExportedObject.t =
  let graph = state.graph in
    ...
    | Object _ ->
      ...
      List.iter
        (fun property ->
          ...
          let object'' = constructobj state _L in
...
let rec program (mode : Mode.t) (is_verbose : bool) (config_path : string)
    (program : m Program.t) : Graph.t * ExportedObject.t * ExternalReferences.t
    =
  verbose := is_verbose;
  ...
  let init_state = BuildMDG.init (Program.functions program) in
  let (state, analysis) = BuildMDG.run init_state (Program.body program) in
