  ...
  and analyse_simple (state : State.t) (analysis : Auxiliary.t ref)
    ...
    | _, AssignFunction { left; id; body; _ } ->
      let func_id : Functions.Id.t =
        { uid = id; name = Identifier.get_name left }
      in
    ...
    | loc, StaticLookup { left; _object; property; id; _ } ->
      let add_node' : location -> unit =
       fun abs_loc ->
        add_node abs_loc (property_lookup_name left _object property) loc
      in
    ...
    | loc, DynmicLookup { left; _object; property; id } ->
      let add_node' : location -> unit =
       fun abs_loc ->
        add_node abs_loc (property_lookup_name left _object "*") loc
      in
    ...
    | loc, DynmicUpdate { _object; property; right; id } ->
      ...
      let _L1, _L2, _L3 =
        (eval_expr _object, eval_expr property, eval_expr right)
      in
  ...
  and analyse_method_call (state : State.t) (loc : Location.t)
    ...
    ( if property != "*" then
        ...
        if LocationSet.cardinal _Lfunc = 1 then
          ...
          | Some { type_ = Function (_, params); _ } ->
            ();
            List.iteri
              (fun i _Ls ->
                let param_name =
...
let add_taink_sink (graph : Graph.t) (loc : location) (node : Graph.Node.t)
...
let add_taint_sinks (state : State.t) (config : Config.t)
  (ext_calls : ExternalReferences.t) : unit =
  let graph = state.graph in

  Graph.iter_nodes
    (fun loc node ->
      match Graph.Node.type_ node with
      | Call callee ->
        (* function sink *)
  ...
  if Mode.is_basic mode then
    Graph.iter_nodes
      (fun location node ->
        ...
        | Function _ ->
          ...
          EdgeSet.iter
            (fun (edge : Edge.t) ->
              match Graph.Edge.type_ edge with
              | Edge.Parameter _ ->
                add_taint_edge l_tsource (Graph.Edge.to_ edge)
              | _ -> () )
            edges
        | _ -> () )
      graph
    ...
    List.iter
      (fun l_func ->
        ...
        match Graph.Node.type_ func_node with
        | Function _ ->
...
let rec construct_object ({ graph; _ } as state : State.t) (loc : LocationSet.t)
  : ExportedObject.t =
  if LocationSet.cardinal loc = 1 then
    let loc = LocationSet.min_elt loc in
    let node = Graph.find_node graph loc in
    match Graph.Node.type_ node with
    | Object _ ->
...
let buildExportsObject (state : State.t) (info : buildExportsObject) :
  ExportedObject.t =
...
let get_analysis_output (result : AnalysisType.t) :
...
let program (mode : Mode.t) (is_verbose : bool) (config_path : string)
  ...
  let exportsObjectInfo, config, external_calls =
    get_analysis_output (Analysis.finish analysis)
  in
