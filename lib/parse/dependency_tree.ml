...
let get_main_file (filename : string) (mode : Mode.t) : string =
  let filename = Graphjs_std.File_system.real_path filename in
  match mode with
...
let generate (filename : string) (mode : Mode.t) =
  ...
  let* output =
    match mode with
    | Multi_file -> Graphjs_std.File_system.run_command' (generate_dt main_file)
