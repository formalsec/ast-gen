        ...
        List.iter
          (fun sink ->
            ...
            | "new" ->
              ...
              let new_sink : newSink = { sink = sink_name; args } in
              news' := new_sink :: !news'
            | "function" ->
              let args = List.map to_int (sink |> member "args" |> to_list) in
              let func_sink : functionSink = { sink = sink_name; args } in
              functions' := func_sink :: !functions'
            | "package" ->
              let packages =
                List.map to_package (sink |> member "packages" |> to_list) in
              let package_sink : packageSink = { sink = sink_name; packages } in
  ...
  if sources != `Null then
    List.iter
      (fun source ->
        ...
        | "package" ->
          let packages =
            List.map to_package (source |> member "packages" |> to_list) in
          let package_source : packageSource =
            { source = source_name; packages } in
...
let get_function_sink_info (config : t) (func_name : string) :
    functionSink option =
  let sink_infos =
    List.filter Fun.(( = ) func_name << get_function_sink_name) config.functions
  in
  List.nth_opt sink_infos 0

let get_package_sink_info (config : t) (package_name : string)
    (method_name : string) : package option =
  let method_sink =
    List.filter
      (fun package_sink -> package_sink.sink = method_name)
      config.packageSinks
    |> Fun.flip List.nth_opt 0 in
  Option.apply
    (fun (method_sink : packageSink) ->
      let package =
        List.filter
          (fun package -> package.package = package_name)
          method_sink.packages
        |> Fun.flip List.nth_opt 0 in
      package )
    ~default:None method_sink

let get_package_source_info (config : t) (package : string) (property : string)
    : packageSource option =
  let source_infos =
    List.filter
      (fun source_info ->
        source_info.source = property
        && List.exists (fun pkg -> pkg.package = package) source_info.packages
        )
      config.packageSources in
