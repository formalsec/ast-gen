...
module Dot = struct
  ...
  let convert_graph (graph : Mdg.t) : G.t =
    let result = ref G.empty in 
    Mdg.iter_edges (fun from edge -> 
        result := G.add_vertex !result from;
        let edge = G.E.create from (Mdg.Edge.label edge) (Mdg.Edge.get_to edge) in 
        result := G.add_edge_e !result edge;
    ) graph;
  
    !result
  
  let output (file_path : string) (graph : Mdg.t) : unit = 
...
module CSV = struct
  let output (file_path : string) (graph : Mdg.t) : unit =
    ...
    Mdg.iter_nodes (fun _ node_info  -> 
      let info = [
    ...
    Mdg.iter_edges (fun loc edge -> 
      let from_id = Mdg.get_node_id graph loc in 
      let to_id = Mdg.get_node_id graph (Edge.get_to edge) in 
    ...
    let edges = string_of_int (Mdg.num_edges graph) in 
    let nodes = string_of_int (Mdg.num_nodes graph) in
