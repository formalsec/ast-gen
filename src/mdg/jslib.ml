...
module LookupInterceptor = struct
  ...
  let update_nodejs_exports (state : State.t) : Node.t option -> Node.t =
    ...
    | None ->
      let exports_jslib = module_name "exports" state.curr_file in
...
module CallInterceptor = struct
  let get_module_name (state : State.t) (ls_args : Node.Set.t list) :
      Fpath.t option =
    let root = fst (Fpath.split_base state.curr_path) in
    match ls_args with
  ...
  let require (cb_build_file : cb_build_file) (state : State.t)
      (retn_name : string) (_ : Node.t) (_ : Node.t) (_ : Node.t)
      (ls_args : Node.Set.t list) : State.t =
    match get_module_name state ls_args with
    | None -> state
    | Some path ->
...
let initialize_module (state : State.t) : State.t =
  let name = "module" in
  let name_jslib = module_name "module" state.curr_file in
...
let initialize_exports (state : State.t) : State.t =
  ...
  let name_store = "exports" in
  let name_jslib = module_name "exports" state.curr_file in
  ...
  Store.replace state.store name_store ls_exports;
  state
