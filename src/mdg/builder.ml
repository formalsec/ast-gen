...
let add_static_orig_object_property (state : State.t) (meta : 'm)
  ...
  if not (Mdg.has_property state.mdg l_orig prop) then
    let l_node = State.add_object_node state meta name in
    State.add_property_edge state l_orig l_node prop

let add_dynamic_orig_object_property (state : State.t) (meta : 'm)
...
let static_nv_strong_update (state : State.t) (meta : 'm) (name : string)
...
let static_nv_weak_update (state : State.t) (meta : 'm) (name : string)
...
let dynamic_nv_strong_update (state : State.t) (meta : 'm) (name : string)
...
let dynamic_nv_weak_update (state : State.t) (meta : 'm) (name : string)
...
let add_static_new_object_version (state : State.t) (meta : 'm) (name : string)
    (ls_obj : Node.Set.t) (prop : string option) : Node.Set.t =
  let get_obj_f = Node.Set.choose in
  match Node.Set.cardinal ls_obj with
  | 0 -> Node.Set.empty
  | 1 -> static_nv_strong_update state meta name (get_obj_f ls_obj) prop
  | _ -> static_nv_weak_update state meta name ls_obj prop

let add_dynamic_new_object_version (state : State.t) (meta : 'm) (name : string)
    (ls_obj : Node.Set.t) (ls_prop : Node.Set.t) : Node.Set.t =
  let get_obj_f = Node.Set.choose in
  match Node.Set.cardinal ls_obj with
  | 0 -> Node.Set.empty
  | 1 -> dynamic_nv_strong_update state meta name (get_obj_f ls_obj) ls_prop
  | _ -> dynamic_nv_weak_update state meta name ls_obj ls_prop

let add_function_caller (state : State.t) (call_meta : 'm) (retn_meta : 'm)
    (call_name : string) (retn_name : string) (ls_args : Node.Set.t list) :
    Node.t * Node.t =
  let l_call = State.add_call_node state call_meta call_name in
  let l_retn = State.add_object_node state retn_meta retn_name in
...
and initialize_hoisted_function (state : State.t) : 'm Statement.t -> State.t =
  function
  | { el = `AssignFunctionDefinition fundef; md } when fundef.hoisted ->
    build_assign_function_definition state md fundef.left fundef.params
      fundef.body
  | _ -> state

and build_assign_simple (state : State.t) (_ : 'm) (left : 'm LeftValue.t)
    (right : 'm Expression.t) : State.t =
  let ls_right = eval_expr state right in
  update_scope state left ls_right;
  state

and build_assign_new (state : State.t) (meta : 'm) (left : 'm LeftValue.t) :
    State.t =
  let l_node = State.add_object_node state meta (LeftValue.name left) in
  update_scope state left (Node.Set.singleton l_node);
  state

and build_assign_unopt (state : State.t) (meta : 'm) (left : 'm LeftValue.t)
    (arg : 'm Expression.t) : State.t =
...
and build_assign_binopt (state : State.t) (meta : 'm) (left : 'm LeftValue.t)
    (arg1 : 'm Expression.t) (arg2 : 'm Expression.t) : State.t =
...
and build_assign_yield (state : State.t) (_meta : 'm) (_left : 'm LeftValue.t)
    (_arg : 'm Expression.t option) (_delegate : bool) : State.t =
...
and build_static_lookup (state : State.t) (meta : 'm) (left : 'm LeftValue.t)
    (obj : 'm Expression.t) (prop : 'm Prop.t) : State.t =
  ...
  add_static_orig_object_property state meta name ls_obj prop';
  let ls_fields = lookup_property state ls_obj prop' in
  update_scope state left ls_fields;
  state

and build_dynamic_lookup (state : State.t) (meta : 'm) (left : 'm LeftValue.t)
    (obj : 'm Expression.t) (prop : 'm Expression.t) : State.t =
  let ls_obj = eval_expr state obj in
  ...
  add_dynamic_orig_object_property state meta name ls_obj ls_prop;
  let ls_fields = lookup_property state ls_obj None in
  update_scope state left ls_fields;
  state

and build_static_update (state : State.t) (meta : 'm) (obj : 'm Expression.t)
    (prop : 'm Prop.t) (right : 'm Expression.t) : State.t =
  (* FIXME: resolve the issue of assigning a value to a literal property *)
  let ls_obj = eval_expr state obj in
  let ls_right = eval_expr state right in
  let prop' = Some (Prop.name prop) in
  let name = object_name ls_obj obj in
  let ls_new = add_static_new_object_version state meta name ls_obj prop' in
...
and build_dynamic_update (state : State.t) (meta : 'm) (obj : 'm Expression.t)
    (prop : 'm Expression.t) (right : 'm Expression.t) : State.t =
  (* FIXME: resolve the issue of assigning a value to a literal property *)
  let ls_obj = eval_expr state obj in
  let ls_prop = eval_expr state prop in
  let ls_right = eval_expr state right in
  let name = object_name ls_obj obj in
  let ls_new = add_dynamic_new_object_version state meta name ls_obj ls_prop in
  ( Fun.flip Node.Set.iter ls_new @@ fun l_new ->
    Fun.flip Node.Set.iter ls_right @@ fun l_right ->
    State.add_property_edge state l_new l_right None );
  state

and build_static_delete (state : State.t) (_meta : 'm) (_left : 'm LeftValue.t)
    (_obj : 'm Expression.t) (_prop : 'm Prop.t) : State.t =
  (* TODO: implement the static delete construct *)
  (* we treat static property deletes as assigning undefined to the static property *)
  state

and build_dynamic_delete (state : State.t) (_meta : 'm) (_left : 'm LeftValue.t)
    (_obj : 'm Expression.t) (_prop : 'm Expression.t) : State.t =
...
and build_function_call (state : State.t) (meta : 'm) (left : 'm LeftValue.t)
    (callee : 'm Identifier.t) (args : 'm Expression.t list) : State.t =
  (* TODO: handle dynamic function calls (callee evaluates to the literal object) *)
  let (call_name, retn_name) = (Identifier.name callee, LeftValue.name left) in
  let ls_funcs = Store.retrieve state.store call_name in
  let funcs = available_functions state ls_funcs in
  let ls_args = List.map (eval_expr state) args in
  let (l_call, l_retn) =
    add_function_caller state callee.md meta call_name retn_name ls_args in
  add_call_connections state l_call ls_args funcs;
  update_scope state left (Node.Set.singleton l_retn);
  state

and build_static_method_call (state : State.t) (meta : 'm)
    (left : 'm LeftValue.t) (obj : 'm Expression.t) (prop : 'm Prop.t)
    (args : 'm Expression.t list) : State.t =
  let ls_obj = eval_expr state obj in
  let ls_args = List.map (eval_expr state) args in
  let prop' = Some (Prop.str prop) in
  let call_name = object_property_name ls_obj obj prop' in
  let retn_name = object_lookup_name left ls_obj obj prop' in
  let lookup_meta = Region.combine obj.md prop.md in
  (* TODO: add a flag to only create property on orig when needed *)
  add_static_orig_object_property state lookup_meta call_name ls_obj prop';
  let ls_mthds = lookup_property state ls_obj prop' in
  let mthds = available_functions state ls_mthds in
  let (l_call, l_retn) =
    add_function_caller state obj.md meta call_name retn_name ls_args in
  add_call_connections state l_call ls_args mthds;
  (* TODO: properly set the this binding *)
  update_scope state left (Node.Set.singleton l_retn);
  state

and build_dynamic_method_call (state : State.t) (meta : 'm)
    (left : 'm LeftValue.t) (obj : 'm Expression.t) (prop : 'm Expression.t)
    (args : 'm Expression.t list) : State.t =
  let ls_obj = eval_expr state obj in
  let ls_prop = eval_expr state prop in
  let ls_args = List.map (eval_expr state) args in
  let call_name = object_property_name ls_obj obj None in
  let retn_name = object_lookup_name left ls_obj obj None in
  let lookup_meta = Region.combine obj.md prop.md in
  (* TODO: check what to do with dynamic properties, and where to add them *)
  add_dynamic_orig_object_property state lookup_meta call_name ls_obj ls_prop;
...
and build_if (state : State.t) (_ : 'm) (consequent : 'm Statement.t list)
...
and build_switch (state : State.t) (_ : 'm) (cases : 'm SwitchCase.t list) :
    State.t =
...
and build_loop (state : State.t) (meta : 'm) (body : 'm Statement.t list) :
    State.t =
  ...
  if Store.equal state'.store store' then state'
  else build_loop state' meta body

and build_assign_function_definition (state : State.t) (meta : 'm)
    (left : 'm LeftValue.t) (params : 'm Identifier.t list)
    (body : 'm Statement.t list) : State.t =
  let func_name = LeftValue.name left in
  let param_names = List.map Identifier.name params in
  let l_func = State.add_function_node state meta func_name param_names in
  ...
  ( Fun.flip List.iteri params @@ fun idx param ->
    ...
    let l_param = State.add_parameter_node state param.md idx' param_name in
...
and build_loop_break (state : State.t) (_meta : 'm)
    (_label : 'm Identifier.t option) : State.t =
  (* TODO: implement flow control to the builder *)
  (* TODO: implement labels on the loop breaks *)
  state

and build_try (state : State.t) (_meta : 'm) (body : 'm Statement.t list)
...
and build_with (state : State.t) (_meta : 'm) (_expr : 'm Expression.t)
    (body : 'm Statement.t list) : State.t =
  (* TODO: implement the with construct *)
  (* extend the current scope with the statement's expression *)
  build_sequence state body

and build_labeled (state : State.t) (_meta : 'm) (_label : 'm Identifier.t)
    (body : 'm Statement.t list) : State.t =
...
and build_return (state : State.t) (_ : 'm) (arg : 'm Expression.t option) :
    State.t =
  (* TODO: implement flow control to the builder *)
...
and build_throw (state : State.t) (_ : 'm) (_arg : 'm Expression.t) : State.t =
...
and build_statement (state : State.t) (stmt : 'm Statement.t) : State.t =
  match stmt.el with
  | `AssignSimple { left; right } ->
    build_assign_simple state stmt.md left right
  | `AssignNewObject { left } | `AssignNewArray { left } ->
    build_assign_new state stmt.md left
  | `AssignUnopt { left; arg; _ } -> build_assign_unopt state stmt.md left arg
  | `AssignBinopt { left; arg1; arg2; _ } ->
    build_assign_binopt state stmt.md left arg1 arg2
  | `AssignYield { left; arg; delegate } ->
    build_assign_yield state stmt.md left arg delegate
  | `StaticLookup { left; obj; prop } ->
    build_static_lookup state stmt.md left obj prop
  | `DynamicLookup { left; obj; prop } ->
    build_dynamic_lookup state stmt.md left obj prop
  | `StaticUpdate { obj; prop; right } ->
    build_static_update state stmt.md obj prop right
  | `DynamicUpdate { obj; prop; right } ->
    build_dynamic_update state stmt.md obj prop right
  | `StaticDelete { left; obj; prop } ->
    build_static_delete state stmt.md left obj prop
  | `DynamicDelete { left; obj; prop } ->
    build_dynamic_delete state stmt.md left obj prop
  | `AssignNewCall { left; callee; args }
  | `AssignFunctionCall { left; callee; args } ->
    build_function_call state stmt.md left callee args
  | `AssignStaticMethodCall { left; obj; prop; args } ->
    build_static_method_call state stmt.md left obj prop args
  | `AssignDynamicMethodCall { left; obj; prop; args } ->
    build_dynamic_method_call state stmt.md left obj prop args
  ...
  | `AssignFunctionDefinition { left; params; body; _ } ->
    build_assign_function_definition state stmt.md left params body
  | `If { consequent; alternate; _ } ->
    build_if state stmt.md consequent alternate
  | `Switch { cases; _ } -> build_switch state stmt.md cases
  | `While { body; _ } | `ForIn { body; _ } | `ForOf { body; _ } ->
    build_loop state stmt.md body
  | `Break { label } | `Continue { label } ->
    build_loop_break state stmt.md label
  | `Return { arg } -> build_return state stmt.md arg
  | `Throw { arg } -> build_throw state stmt.md arg
  | `Try { body; handler; finalizer } ->
    build_try state stmt.md body handler finalizer
  | `With { expr; body } -> build_with state stmt.md expr body
  | `Labeled { label; body } -> build_labeled state stmt.md label body
