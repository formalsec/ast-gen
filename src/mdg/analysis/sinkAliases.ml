...
module Analysis (Init : InitConfig) : AbstractAnalysis.T = struct
    ...
    match statement with
      | _, AssignSimple {left; right} -> 
        ...
        Option.apply (fun right ->
          let sink = Tainted_config.get_function_sink_info config right in 
          Option.apply (fun (sink : functionSink) ->
            let alias = Identifier.get_name left in 
            Tainted_config.add_function_sink config {sink = alias; args = sink.args}
          ) ~default:config sink;
      ...
      | _, StaticLookup {left; _object; property; _} -> 
        ...
        Option.apply (fun obj ->
          let package = Tainted_config.get_package_sink_info config obj property in 
          Option.apply (fun (package : package) ->
            let alias = Identifier.get_name left in 
            Tainted_config.add_function_sink config {sink = alias; args = package.args}
  ...
  let init () : t = Tainted_config.read Init.filename
