...
let add_tainted_sink (mdg : Mdg.t) (jslib : t)
    (make_generic_sink_f : 'a -> Taint.sink) (store : Store_layered.t)
    (generic_sink : 'a) : Store_layered.t =
  ...
  Store_layered.replace store sink.name (Node.Set.singleton l_sink)

let initialize_tainted_sinks (tconf : Taint_config.t) (mdg : Mdg.t)
    (store : Store_layered.t) (jslib : t) : Store_layered.t =
  List.fold_left
    (add_tainted_sink mdg jslib Taint.function_sink)
    store tconf.f_sinks

let initialize_module (mdg : Mdg.t) (store : Store_layered.t) (jslib : t)
    (file : Fpath.t option) (l_parent : Node.t option) : Store_layered.t =
  let name = "module" in
  ...
  Store_layered.replace store name (Node.Set.singleton l_module)

let initialize_exports (mdg : Mdg.t) (store : Store_layered.t) (jslib : t)
    (file : Fpath.t option) (l_parent : Node.t option) : Store_layered.t =
  let name = "exports" in
  ...
  Store_layered.replace store name (Node.Set.singleton l_exports)

let create (tconf : Taint_config.t) (mdg : Mdg.t) (store : Store_layered.t) :
    Store_layered.t * t =
  let jslib = Hashtbl.create Config.(!dflt_htbl_sz) in
  let store = initialize_tainted_sinks tconf mdg store jslib in
  (store, jslib)

let initialize (mdg : Mdg.t) (store : Store_layered.t) (jslib : t)
    (file : Fpath.t option) (l_parent : Node.t option) : Store_layered.t =
  let store = initialize_module mdg store jslib file l_parent in
